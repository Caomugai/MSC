...
#pragma region Band ratio method
	for (int i = 0; i < bandindexes.size(); i++) // Band ratio method for fire detection
	{
		for (int j = 0; j < bandindexes.size(); j++)
		{
			if (j != i) // Use different bands for ratio calculation
			{
				int linshi_band[100][2];        // Temporary storage for band pairs
				float linshi_threshold[100];    // Temporary storage for single thresholds (1D array)
				float linshi_zhengquelv[100];   // Temporary storage for accuracy rates
				int ncount = 0;

				// Use vectors for automatic memory management
				std::vector<float> ratiofire(linefire);
				std::vector<float> ratioclear(lineclear);

				// Calculate band ratios for clear sky pixels
				for (int y = 0; y < lineclear; y++)
				{
					// Avoid division by zero
					if (fabs(dataset_clear[j][y]) > 1e-6)
					{
						ratioclear[y] = dataset_clear[i][y] / dataset_clear[j][y];
					}
					else
					{
						ratioclear[y] = 0;
				...
				// Calculate band ratios for fire pixels
				for (int x = 0; x < linefire; x++)
				{
					// Avoid division by zero
					if (fabs(dataset_fire[j][x]) > 1e-6)
					{
						ratiofire[x] = dataset_fire[i][x] / dataset_fire[j][x];
					}
					else
					{
						ratiofire[x] = 0;
				...
				// Test different single threshold values
				for (float threshold = 0.0f; threshold <= 5.0f; threshold += 0.05f)
				{
					int m = 0; // Count of fire pixels above threshold
					int n = 0; // Count of clear sky pixels above threshold (false alarms)

					// Count fire pixels with ratio above threshold
					for (int x = 0; x < linefire; x++)
					{
						if (ratiofire[x] > threshold)
						{
							m++;
						}
					}

					// Count clear sky pixels with ratio above threshold
					for (int y = 0; y < lineclear; y++)
					{
						if (ratioclear[y] > threshold)
						{
							n++;
						}
					}
					...
					float nnn = float(n) / float(lineclear); // False alarm rate
					float mmm = float(m) / float(linefire); // Fire detection accuracy
					float accuracy = mmm - nnn;              // Net accuracy

					// Criteria: false alarm rate < 3% and net accuracy > 85%
					if (nnn < 0.05 && accuracy > 0.85)
					{
						linshi_band[ncount][0] = i;
						linshi_band[ncount][1] = j;
						linshi_threshold[ncount] = threshold; // Store single threshold
						linshi_zhengquelv[ncount] = accuracy;
						ncount++;

						if (ncount >= 100) break; // Prevent array overflow
					}
				}
				...
				{
					int best_index = 0;
					float best_accuracy = linshi_zhengquelv[0];
					for (int jj = 1; jj < ncount; jj++)
					{
						if (linshi_zhengquelv[jj] > best_accuracy)
						{
							best_accuracy = linshi_zhengquelv[jj];
							best_index = jj;
						}
					}
					...
					if (best_accuracy > 0)
					{
						band_bizhi[n_bizhi][0] = i + 1;
						band_bizhi[n_bizhi][1] = j + 1;
						threshold_bizhi[n_bizhi][0] = linshi_threshold[best_index]; // Single threshold
						threshold_bizhi[n_bizhi][1] = 0; // Set second threshold to 0 (unused)
						zhengquel_bizhi[n_bizhi] = best_accuracy;

						outfile << "ratio_band" << " " <<
							atoi(bandindexes[i].c_str()) << " " << atoi(bandindexes[j].c_str()) << " " <<
							threshold_bizhi[n_bizhi][0] << " " << zhengquel_bizhi[n_bizhi] << endl;
						n_bizhi++;
					}
				}
				// Vectors automatically deallocated, no need for manual delete
			}
		}
	}
#pragma endregion Band ratio method

#pragma region Band normalized index method
	// Band normalized index method for fire detection
	for (int i = 0; i < bandindexes.size(); i++)
	{
		for (int j = 0; j < bandindexes.size(); j++)
		{
			if (j != i) // Use different bands for index calculation
			{
				int linshi_band[100][2];        // Temporary storage for band pairs
				float linshi_threshold[100];    // Temporary storage for single thresholds (1D array)
				float linshi_zhengquelv[100];   // Temporary storage for accuracy rates
				...
				// Use vectors for automatic memory management
				std::vector<float> indexFire(linefire);
				std::vector<float> indexBright(lineclear);

				// Calculate normalized index for clear sky pixels: (band_i - band_j) / (band_i + band_j)
				for (int y = 0; y < lineclear; y++)
				{
					float denominator = dataset_clear[i][y] + dataset_clear[j][y];
					if (fabs(denominator) > 1e-6) // Avoid division by zero
					{
						indexBright[y] = (dataset_clear[i][y] - dataset_clear[j][y]) / denominator;
					}
					else
					{
						indexBright[y] = 0;
				...
				// Calculate normalized index for fire pixels
				for (int x = 0; x < linefire; x++)
				{
					float denominator = dataset_fire[i][x] + dataset_fire[j][x];
					if (fabs(denominator) > 1e-6) // Avoid division by zero
					{
						indexFire[x] = (dataset_fire[i][x] - dataset_fire[j][x]) / denominator;
					}
					else
					{
						indexFire[x] = 0;
				...
				// Test different single threshold values for normalized index
				// Normalized index range is typically [-1, 1], but we test a wider range for safety
				for (float threshold = -1.0f; threshold <= 1.0f; threshold += 0.01f)
				{
					int m = 0; // Count of fire pixels above threshold
					int n = 0; // Count of clear sky pixels above threshold (false alarms)

					// Count fire pixels with normalized index above threshold
					for (int x = 0; x < linefire; x++)
					{
						if (indexFire[x] > threshold)
						{
							m++;
						}
					}

					// Count clear sky pixels with normalized index above threshold
					for (int y = 0; y < lineclear; y++)
					{
						if (indexBright[y] > threshold)
						{
							n++;
						}
					}
				...
				if (ncount > 0)
				{
					...
					if (best_accuracy > 0)
					{
						band_zhishu[n_zhishu][0] = i + 1;
						band_zhishu[n_zhishu][1] = j + 1;
						threshold_zhishu[n_zhishu][0] = linshi_threshold[best_index]; // Single threshold
						threshold_zhishu[n_zhishu][1] = 0; // Set second threshold to 0 (unused)
						zhengquel_zhishu[n_zhishu] = best_accuracy;

						outfile << "Index_band" << " " <<
							atoi(bandindexes[i].c_str()) << " " << atoi(bandindexes[j].c_str()) << " " <<
							threshold_zhishu[n_zhishu][0] << " " << zhengquel_zhishu[n_zhishu] << endl;
						n_zhishu++;
					}
				}
				// Vectors automatically deallocated, no need for manual delete
...
#pragma endregion Band normalized index method
